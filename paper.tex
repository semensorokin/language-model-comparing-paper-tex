...
\end{itemize}
It is important to note that the resulting model is very dependent on the quality of the based LM. So a possibility to compare models is also in demand and we propose a new comprehensive way to explore properties of different language models.
...
Our approach is based on the specific Russian corpus manually collected for the purposes of experiments with eye-tracking. The corpus was used for conducting a cloze test in the paper by Laurinavichyute et al. \cite{laurinavichyute}. This fact assumes that we have not only correct word for predicting left context of different lengths, but also probabilities for each predicted word, computed according to the result of the cloze task experiment. The LM based on these answers we will call "human-like". So the uniqueness of the research is that we can compare artificial language models with the model which approximates real human expectations about the next word for a given sequence.

It is important to note that cloze tasks require a major time commitment and are financially expensive, therefore one of the goals of this study is to find out whether the actual human respondents can be replaced by an artificial language model trained on a big corpus, or, whether language models can simulate human performance on this task.

In our work, we hope to compare different language models to see which of them can be used for an automated cloze task, i. e. a model that is able to predict/guess the next missing word or POS in a sentence.
